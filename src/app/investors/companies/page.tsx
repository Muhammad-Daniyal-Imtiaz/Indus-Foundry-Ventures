...
export default function CompanyInvestorsPage() {
  ...
  const filteredCompanies = useMemo(() => {
    return companyInvestors.filter(c => {
      ...
      if (chequeFilter !== "All") {
        const minVal = c.minCheque || 0;
        const maxVal = c.maxCheque || 999999999;
  ...
  }, [companyInvestors, searchTerm, selectedCountry, selectedType, selectedStage, chequeFilter]);
      ...
      <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 relative">
                ...
                <input
                  type="text"
                  placeholder="Search VCs by fund name, investment thesis, hq..."
                  value={searchTerm}
                  onChange={(e) => handleSearchChange(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-[#05070c] border border-white/5 text-white placeholder-slate-500 text-xs font-semibold rounded-xl focus:outline-none focus:border-cyan-500/30 transition-all outline-none"
                />
              </div>
              ...
              <div>
                <select
                  ...
                  className="w-full bg-[#05070c] border border-white/5 focus:border-cyan-500/30 text-white text-xs px-4 py-3 rounded-xl outline-none transition-all cursor-pointer font-bold"
                >
                  <option value="All">All VC Categories</option>
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-[10.5px] font-bold text-slate-500 uppercase tracking-widest font-mono border-b border-white/5 pb-4">
          ...
          {selectedCountry === "Pakistan" ? (
            <span className="text-emerald-400">Showing {pakCompaniesCount} Pakistan-Active VCs</span>
          ) : (
            <span className="text-cyan-400">Target Focus: {selectedCountry}</span>
          )}
        </div>
